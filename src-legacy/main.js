...
import { resolveViewFromPath, navigateToView as goToView } from "./navigation.js";
import {
  mountArchiveEmbedsForView,
  initArchiveEmbedTap,
  initArchiveDaysOverlay,
} from "./archive-embed.js";
import {
  renderLandingArchivePreview,
  renderStatistikaPage,
  bindStatTabs,
} from "./statistika.js";
import {
  renderZavarovalnicaPage,
  isZavarovalnicaView,
  resultsPanelClass,
  resultsBackNav,
} from "./zavarovalnica.js";
import {
  renderWidgetObcinePage,
  bindWidgetObcineControls,
  loadPublicWidgetObcineList,
  initWidgetObcinePage,
} from "./widget-obcine.js";
import { mountGoogleSignInButton } from "./auth-google.js";
  ...
  return `
    <header class="site-header${logged ? " site-header--logged-in" : " site-header--guest"}">
      <div class="site-header__bar">
        ...
        <nav class="site-nav site-nav--desktop" aria-label="Glavna navigacija">
          ...
          <div class="site-nav__auth nav-actions">
            ${
              logged
...
function renderSearchOverlay() {
  const searchView =
    state.view === "landing" || state.view === "zavarovalnica";
  const active = state.loading || (state.preview && searchView);
  if (!active) return "";
...
function renderLanding() {
  ...
  return `
    <section class="hero">
      ...
      <div class="search-card${overlayBusy ? " search-card--busy" : ""}">
        ...
        <div class="search-card-body">
          ...
          <input id="location-input" class="search-input" type="text" placeholder="npr. Celje, Slovenska 1" autocomplete="off" value="${escapeHtml(state.locationQuery || state.selected?.label || "")}" ${state.loading ? "disabled" : ""} />
    ...
    <section class="features">
      <a href="/pomoc-pri-zavarovalnici" class="feature feature--link" data-nav="zavarovalnica">
        ${iconInsurance()}<h4>Pomoč pri zavarovalnici</h4><p>Podatki za dokazovanje bližnjih udarov strel pri zavrnitvi škode.</p>
      </a>
      <a href="/statistika#zemljevid" class="feature feature--link" data-nav="statistika" data-stat-tab="zemljevid">
        ${iconMap()}<h4>Zemljevid udarov</h4><p>Pregled strel okoli vašega doma na interaktivnem zemljevidu.</p>
      </a>
      <a href="/statistika" class="feature feature--link" data-nav="statistika" data-stat-tab="grafi">
        ${iconShield()}<h4>Arhiv strel</h4><p>Statistika udarov strel po Sloveniji — dnevni in urni pregled.</p>
      </a>
    </section>
    ${renderLandingArchivePreview(state, escapeHtml)}`;
}
...
function renderResults() {
  ...
  return `
    <section class="results-panel${resultsPanelClass(state)}">
      ...
      <div class="results-actions">
        ...
        <button type="button" class="btn btn-ghost" data-nav="${resultsBackNav(state)}">Nova preiskava</button>
      </div>
    </section>`;
}
    ...
    <div class="modal-overlay" id="auth-modal">
      <div class="modal">
        ...
        <form id="auth-form">
          ...
          <button type="submit" class="btn btn-primary btn-block">${isLogin ? "Prijava" : "Ustvari račun"}</button>
        </form>
        <div class="auth-divider"><span>ali</span></div>
        <div id="strelko-google-signin" class="auth-google-wrap"></div>
...
function navigateToView(view, opts = {}) {
  goToView(state, view, { ...opts, render });
}
...
function render() {
  ...
  else if (state.view === "zavarovalnica") {
    body = renderZavarovalnicaPage(state, {
      escapeHtml,
      renderSearchOverlay,
      renderSelectedPlace,
    });
  } else if (state.view === "statistika") body = renderStatistikaPage(state);
  else if (state.view === "widget-obcine") body = renderWidgetObcinePage(state, escapeHtml);
  ...
  bindEvents();
  bindStatTabs(state, { render, navigateToView });
  if (state.view === "widget-obcine") {
    bindWidgetObcineControls(state, { $, render });
    initWidgetObcinePage(state, { render, $ }).catch(() => {});
  }

  initArchiveEmbedTap();
  initArchiveDaysOverlay();
  mountArchiveEmbedsForView(state.view, state.statistikaTab, !!state.user);
...
function bindEvents() {
  ...
  input?.addEventListener("input", () => {
    ...
    if (state.selected && state.selected.label !== input.value.trim()) {
      state.selected = null;
    }
  ...
  document.querySelectorAll("[data-nav]").forEach((el) => {
    el.addEventListener("click", (e) => {
      e.preventDefault();
      const nav = el.dataset.nav || "landing";
      const statTab = el.dataset.statTab;
      state.preview = null;
      ...
      if (nav === "statistika") {
        navigateToView("statistika", { statTab: statTab || "grafi" });
        return;
      }
      if (nav === "widget-obcine") {
        navigateToView("widget-obcine");
        return;
      }
      if (nav === "zavarovalnica") {
        navigateToView("zavarovalnica");
        return;
      }
      navigateToView("landing");
    });
...
function showAuthModal(mode) {
  ...
  overlay?.querySelectorAll("[data-legal]").forEach((el) => {
    el.addEventListener("click", (e) => {
      e.preventDefault();
      overlay?.remove();
      navigateToLegal(el.dataset.legal);
  ...
  const googleBox = $("#strelko-google-signin");
  mountGoogleSignInButton(googleBox, async (credential) => {
    const errEl = $("#auth-error");
    try {
      ...
      if (errEl) {
        errEl.textContent = ex.message || "Google prijava ni uspela.";
        errEl.classList.remove("hidden");
      }
    }
  }, escapeHtml);
...
async function init() {
  ...
  if (legalId) {
    state.legalPage = legalId;
  } else {
    const { view, statTab } = resolveViewFromPath();
    state.view = view;
  ...
  window.addEventListener("popstate", () => {
    ...
    if (id) {
      ...
      if (resolved.statTab) state.statistikaTab = resolved.statTab;
    }
    render();
  });
  ...
  await loadPublicWidgetObcineList(state, render);
  render();
  if (state.view === "widget-obcine") {
    await initWidgetObcinePage(state, { render, $ });
  }
}
