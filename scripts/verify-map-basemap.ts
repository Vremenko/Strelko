...
assert.ok(mapHtml.includes("(hover: hover) and (pointer: fine)"));
assert.ok(mapHtml.includes("(any-hover: hover) and (any-pointer: fine)"));
...
assert.ok(!mapHtml.includes("setZoomAround"), "brez lastnega pinch setZoomAround");
assert.ok(mapHtml.includes("touchZoom.enable()"));
assert.ok(/touch-action:\s*pan-y\s+pinch-zoom/.test(mapHtml));
